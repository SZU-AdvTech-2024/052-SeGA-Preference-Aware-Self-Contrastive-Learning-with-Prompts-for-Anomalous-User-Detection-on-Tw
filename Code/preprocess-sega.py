...
def Read_Data():
    print("Read data")
    sample_path = "/data2/csq/SeGA-main/Sega/"
    path = "/data2/csq/SeGA-main/Sega/"
    ...
    return fine_label, fine_split, edge, user, lst

def Label_Split_Processed(fine_label, fine_split, user):
...
def Preprocess_Edge(edge,user,lst,hashtag):
    ...
    for index,e in tqdm(discuss_edge.iterrows()):
        sid = e['source_id']
        tid = e['target_id']
        edge_index.append([uid_index[sid],len(index_uid)+len(index_lid)+len(hid_index)])
...
fine_label, fine_split, edge, user, lst= Read_Data()

Initial_User_Embedding(user, 'followers_count', 'random')
Initial_User_Embedding(user, 'following_count', 'random')
Preprocess_Edge(edge, user, lst)
Label_Split_Processed(fine_label, fine_split, user)
